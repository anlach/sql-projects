...
echo "$($PSQL "select wt.name from games join teams as wt on games.winner_id = wt.team_id where year=2014 union select lt.name from games join teams as lt on games.opponent_id = lt.team_id where year = 2014 order by name")"
